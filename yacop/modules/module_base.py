...
class SteenrodModuleBase(CombinatorialFreeModule):
    ...
    def some_elements(self, num=10):
        ...
        for i in range(0, num):
            elm = self.linear_combination(
                (self.monomial(g), c) for (g, c) in zip(gens[i:], cfs)
            )
...
class SteenrodModuleBase_Tensor(CombinatorialFreeModule.Tensor, SteenrodModuleBase):
    ...
    def __classcall_private__(cls, modules, **options):
        modules = sum(
            [
                module._sets
                if isinstance(module, SteenrodModuleBase_Tensor)
                else (module,)
                for module in modules
            ],
            (),
        )
        return super(SteenrodModuleBase_Tensor, cls).__classcall__(
            cls, modules, **options
        )
    ...
    def _repr_term(self, term):
        ...
        if symb is None:
            ...
            symb = tensor.symbol
        return symb.join(
            withbrackets(module.monomial(t)._repr_(), False)
    ...
    def _latex_term(self, term):
        ...
        symb = " \\otimes "
        return symb.join(
            withbrackets(latex(module.monomial(t)), True)
            for (module, t) in zip(self._sets, term)
        )
    ...
    class Element(SteenrodModuleBase.Element, CombinatorialFreeModule.Tensor.Element):
        ...
        def _test_pickling(self, tester=None, **options):
            if self.parent()._can_test_pickling():
                super(SteenrodModuleBase_Tensor.Element, self)._test_pickling(
...
class SteenrodModuleBase_CartesianProduct(
    CombinatorialFreeModule.CartesianProduct, SteenrodModuleBase
):
    def __init__(self, modules, *args, **kwargs):
        CombinatorialFreeModule.CartesianProduct.__init__(
            self, modules, *args, **kwargs
        )
        self._fix_basis_tests()

    def _repr_term(self, elem):
        return "(%s)" % ", ".join(
            mod.monomial(u)._repr_() for (mod, u) in zip(self._sets, elem)
        )

    def _latex_term(self, elem):
        return "(%s)" % ", ".join(
            mod.monomial(u)._latex_() for (mod, u) in zip(self._sets, elem)
        )
    ...
    def dump_element(self, el):
        return str(
            [
                p.dump_element(self.cartesian_projection(n)(el))
                for (n, p) in enumerate(self._sets)
            ]
        )
    ...
    class Element(CombinatorialFreeModule.CartesianProduct.Element):
        ...
        def _test_pickling(self, tester=None, **options):
            if self.parent()._can_test_pickling():
                super(SteenrodModuleBase_CartesianProduct.Element, self)._test_pickling(
                    tester=tester, **options
                )
