...
class FreeModuleBasis(Parent, UniqueRepresentation):
    ...
    def __classcall_private__(
        cls, algebra, gens, subalgebra=None, unstable=None, bbox=None, facade=None
    ):
        if unstable is None:
            unstable = False
        if bbox is None:
            bbox = region()
        return super(FreeModuleBasis, cls).__classcall__(
            cls, algebra, gens, subalgebra, unstable, bbox, facade
        )
    ...
    def __init__(self, algebra, gens, subalgebra, unstable, bbox, facade):
        ...
        if hasattr(gens, "dump_element"):
            ...
            self._dumpfuncs = lambda x: base64.b64encode(dumps(x)), lambda x: loads(
                base64.b64decode(x)
            )
        if self.is_finite():
            ...
            self._algbox = region(
                tmin=0, tmax=self._algebra.top_class().degree(), emin=0, emax=emax, s=0
            )
        else:
    ...
    def is_finite(self):
        return (
            self._algebra.is_finite() or self._trunc.tmax < Infinity
        ) and self._gens.is_finite()
...
class FreeModuleImpl(SteenrodModuleBase):
    ...
    @staticmethod
    def __classcall_private__(
        cls, algebra, gens, subalgebra=None, left_action=None, right_action=None
    ):
        if left_action is None:
            left_action = True
        if right_action is None:
            right_action = False
        return super(FreeModuleImpl, cls).__classcall__(
            cls, algebra, gens, subalgebra, left_action, right_action
        )

    def __init__(
        self, algebra, gens, subalgebra, left_action, right_action, category=None
    ):
        """
            ...
            if category is None:
                if left_action:
                    ...
                    if right_action:
                        ...
                        raise ValueError(
                            "neither left_action, nor right_action specified"
                        )
...
def YacopFreeModule(
    algebra, gens, left_action=True, right_action=False, names=None, *args, **kwargs
):
    """
