...
class MorphModule(SteenrodModuleBase):
   ...
   def __init__(self,f,ambient,category=None,truncate_to=None):
      ...
      if gr.bbox().is_finite('t','e','s') or ambient.basis().is_finite():
         self._card = -1 ;# finite but unknown
      else:
         self._card = Infinity
      ...
      keys = Family(IntegerRange(Integer(0),+Infinity),lambda i:self.__basis_key_by_idx(i),lazy=True)
      import types
      def contains(fam,x):
         try:
            reg,idx = x
   ...
   def _module_basis(self,region):
      assert region.tmax == region.tmin and region.emax==region.emin and region.smax==region.smin
      ans = []
      M = self.ambient()
   ...
   def _basis_walker(self,region=None):
      ...
      class momobasis_walker(Walker):
         def __init__(self,morphmod,region,cardhint):
            ...
            self._card = cardhint
            r = self.reg
            if not r is None and r.tmin == r.tmax and r.emin==r.emax and r.smin==r.smax:
   ...
   def _compute_matrix(self,region):
        ...
        assert region.tmin == region.tmax and region.emin==region.emax and region.smin==region.smax
   ...
   class Element(SteenrodModuleBase.Element):
      ...
      def _test_pickling(self,tester=None,**options):
         if self.parent()._can_test_pickling():
             super(MorphModule.Element,self)._test_pickling(tester=tester,**options)
...
class KernelImpl(MorphModule):
   ...
   def ambient_embedding(self):
      res = self.module_morphism(codomain=self._f.domain(),on_basis = lambda i: self.lift(self.monomial(i)))
   ...
   class Element(MorphModule.Element):
      ...
      def _test_element_in_kernel(self,tester=None,**options):
         ...
         tester.assertTrue(self.parent()._f(self)==0,
               LazyFormat("element %s not in kernel")
                                            %(self,))
...
class ImageImpl(MorphModule):
   ...
   @cached_method
   def ambient_embedding(self):
      res = self.module_morphism(codomain=self._f.codomain(),on_basis = lambda i: self.lift(self.monomial(i)))
...
class CokerImpl(MorphModule):
   ...
   def retraction_morphism(self):
      M = self._f.codomain()
      res = M.module_morphism(codomain=self,on_basis = lambda i: self.retract(M.monomial(i)))
