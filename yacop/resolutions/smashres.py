...
def tclsteenrodop(algebra, str):
    ...
    for smd in str.split("} {"):
        ...
        if algebra.is_generic():
            e = tuple(
                idx for (idx, bit) in enumerate(Integer(op[1]).digits(2)) if bit == 1
            )
            dct[(e, r)] = cf
        else:
            dct[r] = cf
...
class Smasher(Parent, UniqueRepresentation):
    ...
    def __classcall_private__(cls, resolution, module, filename=None, debug=False):
        if filename is None:
            ...
            filename = "file::yacopsr%d.%f?mode=memory&cache=shared" % (
                Smasher._memdbcnt,
                time.time(),
            )
            tempdb = True
            ...
            tempdb = False
        return super(Smasher, cls).__classcall__(
            cls, resolution, module, filename, tempdb, debug
        )

    def __init__(
        self, resolution, module, filename=":memory:", tempdb=False, debug=False
    ):
    ...
    def _find_region(self, reg):
        ...
        reg.smin = max(reg.smin, msmin)
        # we assume resolution.smin == 0
    ...
    def _make_smash_homology(self, reg, dbg=False):
        ...
        """
        self.tcl.eval(
            "smashprod update_homology {%s} %d" % (reg.as_tcl(), self._tfactor)
        )
    ...
    def xxxhomology(self, s=None, n=None, i=None, e=None, nov=None):
        ...
        return Set(
            [
                FormalSumOfDict([(1, Generator(self, x))], parent=self._fsums)
                for x in eval(res)
            ]
        )
    ...
    def smash_basis(self, reg):
        ...
        return list(
            self.make_tensor(
                self._module.load_element(x["modgen"]),
                self.reference_resolution._generator_from_dict(
                    self._resolution.g(id=x["resgen"])
                ),
            )
            for x in eval(res)
        )
    ...
    def fragments(self, querycond="where 1"):
        ...
        cor = self.tcl.eval(
            "coroutine %s smashprod_fragment_iterator {%s}" % (cname, querycond)
        )
    ...
    def xxcycle(self, generator):
        ...
        mat = self._getmatrix(
            "homology", s=generator["s"], i=generator["i"], e=generator["e"]
        )
        # print mat
    ...
    def xxhomology_class(self, cycle):
        ...
        # print eval(res)
        return FormalSum(
            [(cf, self.g(id=gid)) for (cf, gid) in eval(res)], parent=self._fsums
        )
    ...
    def _makeSageModule(self, region):
        ...
        # print todo
        resp = (
            "unset -nocomplain df;unset -nocomplain ar;array set df {};array set ar {};"
        )
        for g in todo["diff"]:
            mg = modgens[g]
...
class SmashResolution(SteenrodModuleBase_Tensor, UniqueRepresentation):
    ...
    def __classcall_private__(
        ...
        if with_worker is None:
            with_worker = True
        return super(SmashResolution, cls).__classcall__(
            cls,
            module,
            resolution,
            filename=filename,
    ...
    def __init__(
        ...
        SteenrodModuleBase_Tensor.__init__(
            self, (module, resolution), category=category
        )
    ...
    def SuspendedObjectsFactory(module, *args, **kwopts):
        ...
        TESTS::
            ...
            Category of tensor products of yacop left modules over sub-Hopf algebra of mod 3 Steenrod algebra, milnor basis, profile function ([1], [2, 2])
        """
        return SmashResolution(
            suspension(module._sets[0], *args, **kwopts), module._sets[1]
        )
...
class SmashResolutionHomology(FreeModuleImpl, UniqueRepresentation):
    ...
    def _lift_homogeneous(self, deg, smd):
        ...
        return par.linear_combination(
            (
                par.linear_combination(
                    list(zip(ba, mat.row(g.split()[1]._dct["num"])))
                ),
                cf,
            )
            for (g, cf) in list(smd)
        )
    ...
    def _retract_homogeneous(self, deg, smd):
        ...
        return self.linear_combination(
            (self.g(deg.s, deg.t, idx), vc) for (idx, vc) in enumerate(ans)
        )
