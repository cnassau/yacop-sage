...
class FiniteGradedSet(Parent,UniqueRepresentation):
   ...
   @staticmethod
   def __classcall_private__(cls,elements=None,tdegree=None,edegree=None,sdegree=None,tesfunc=None,elemlist=None):
      if elements is None:
         elements = ()
      if elemlist is None:
         if tdegree is None:
      ...
      return super(FiniteGradedSet,cls).__classcall__(cls,None,elemlist=tuple(elemlist))

   def __init__(self,elements=None,tdegree=None,edegree=None,sdegree=None,tesfunc=None,**kwargs):
      """
   ...
   def _truncate_region(self,reg):
      ...
      return FiniteGradedSet(res,tesfunc=lambda n: [(x.t,x.e,x.s) for x in (self.degree(n),)][0])
